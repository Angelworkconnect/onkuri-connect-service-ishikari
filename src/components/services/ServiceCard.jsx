...
export default function ServiceCard({ title, description, icon: Icon, color, detail_url, onClick }) {
  return (
    <Card 
      onClick={onClick}
      className={`group relative overflow-hidden bg-white border-0 shadow-sm hover:shadow-lg transition-all duration-300 ${detail_url ? 'cursor-pointer' : ''}`}
    >
      <div className="p-6">

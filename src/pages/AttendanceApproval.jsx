...
export default function AttendanceApproval() {
  // authInfo: null=未確定, {user, isAdmin} = 確定済
  ...
  const authReady = authInfo !== null;

  // 勤怠データ: 管理者=全件, 一般=自分のみ（authReady後に実行）
  const { data: attendanceRecords = [] } = useQuery({
    queryKey: ['attendance-approval', authReady, isAdmin, user?.email],
    queryFn: async () => {
      if (!authReady) return [];
      if (isAdmin) {
        return base44.entities.Attendance.list('-date');
      }
      // 一般スタッフ: 自分のメールアドレスで厳密にフィルタ
  ...
  const { data: allStaff = [] } = useQuery({
    ...
    enabled: authReady && isAdmin, // 管理者のみ取得
  });
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['attendance-approval'] }),
  });
  ...
  const ActionButtons = ({ record }) => (
    <div className="flex gap-1 flex-wrap">
      <Button size="sm" variant="outline" className="text-[#2D4A6F] border-[#2D4A6F]/30" onClick={() => setEditRecord({ ...record, user_name: isAdmin ? getStaffName(record.user_email) : user.full_name })}>
  ...
  const RecordRow = ({ record, showName = false }) => (
    <TableRow>
  ...
  const TableHeaders = ({ showName = false }) => (
    <TableHeader>
      <TableRow>
  ...
  // authReady になるまでローディング表示（isAdmin が確定する前に Tabs を描画しない）
  if (!authReady) {
    return <div className="min-h-screen flex items-center justify-center"><div className="text-slate-400">読み込み中...</div></div>;
  }
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white pb-20">
      ...
      <div className="bg-gradient-to-br from-[#2D4A6F] to-[#1E3A5F] text-white">
        <div className="max-w-6xl mx-auto px-6 py-12">
          <h1 className="text-3xl font-light mb-2">勤怠{isAdmin ? '承認' : '確認'}</h1>
          <p className="text-white/70">{isAdmin ? '職員の勤怠を確認・承認します' : '自分の勤怠記録を確認します'}</p>
        </div>
      </div>
          ...
          <Tabs defaultValue="daily" className="w-full">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              {pendingRecords.length > 0 && (
                <Badge className="bg-yellow-100 text-yellow-700 text-sm px-3 py-1">勤怠承認待ち {pendingRecords.length}件</Badge>
              )}
            </div>
              ...
              {byDay.length === 0 ? (
                ...
                <Card key={date} className="border-0 shadow-lg mb-4">
                  ...
                  <div className="overflow-x-auto">
                    <Table><TableHeaders showName={true} /><TableBody>{records.map(r => <RecordRow key={r.id} record={r} showName={true} />)}</TableBody></Table>
                  </div>
                </Card>
              ))}
            </TabsContent>
            ...
            <TabsContent value="monthly">
              <Card className="border-0 shadow-lg">
                ...
                <div className="overflow-x-auto">
                  <Table><TableHeaders showName={true} /><TableBody>
            ...
            <Card className="border-0 shadow-lg">
              <div className="px-6 py-4 border-b bg-slate-50 flex items-center gap-3">
                ...
                <span className="text-slate-600 text-sm">
                  合計: {formatMinutes(monthRecords.reduce((sum, r) => sum + calculateWorkMinutes(r.clock_in, r.clock_out, r.break_minutes), 0))}
                </span>
              </div>
              <div className="overflow-x-auto">
                <Table>
